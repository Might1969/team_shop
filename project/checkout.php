...
<section class="checkout">
      ...
      if(isset($_GET['get_id'])) {
         ...
         if($select_get->rowCount() > 0) {
            while($fetch_get = $select_get->fetch(PDO::FETCH_ASSOC)) {
               $grand_total += $fetch_get['price'] * 1;  
   ?>
   <div class="flex">
      <img src="uploaded_files/<?= $fetch_get['image']; ?>" class="image" alt="">
      <div>
         <h3 class="name"><?= $fetch_get['name']; ?></h3>
         <p class="price"><span class="styled-dollar">&dollar;</span> <?= $fetch_get['price']; ?> x 1</p>
      </div>
   </div>
   <?php
            }
   ...
   <div class="flex">
      ...
      <div>
         ...
         <p class="price"><span class="styled-dollar">&dollar;</span> <?= $fetch_product['price']; ?> x <?= $fetch_cart['qty']; ?></p>
      </div>
   </div>
   <?php
            }
   ...
   ?>
   <div class="grand-total"><span>grand total :</span><p><span class="styled-dollar">&dollar;</span> <?= $grand_total; ?></p></div>
</div>
